...
def draw_circles2():
    ...
    for k in range(24):##n=24
        center_point=rg.Point((30 + total),100)
        total = 10 + total
        circle = rg.Circle(center_point, radius)
...
def draw_cosines_and_sines():
    ...
    for k in range(101):  ##n=24
        x = math.cos(total) * 80
        y= math.sin(total) * 80
        center_point = rg.Point((200 + x), (200+ y))
        total = 1 + total
        circle = rg.Circle(center_point, radius)
